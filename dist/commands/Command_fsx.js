...
exports.Fsx_Private = function (ws, user_id, type, value) {
    console.log('[Debug]', 'Receive command: Fsx, User_ID:', user_id);
    CQReply_1.send_private_msg(ws, user_id, fsx(type, value));
};
exports.Fsx_Group = function (ws, group_id, type, value) {
    console.log('[Debug]', 'Receive command: Fsx, Group_ID:', group_id);
    CQReply_1.send_group_msg(ws, group_id, fsx(type, value));
};
function fsx(type, value) {
    var version = '版本 5.0 Lv80 等级基数: 3300';
    var base = 3300;
    if (!type || !value) {
    ...
    if (type === '暴击' || type === '暴') {
        var msg = '';
        var perc = (5 + (value - 380) / base * 20).toFixed(1);
        var perv = (140 + (value - 380) / base * 20).toFixed(1);
        if (perc < 0)
            perc = 0;
        if (perv < 0)
            perv = 0;
        ...
        var msg = '';
        var perc = ((value - 380) / base * 55).toFixed(1);
        if (perc < 0)
            perc = 0;
        ...
        var msg = '';
        var perc = ((value - 340) / base * 13).toFixed(1);
        if (perc < 0)
            perc = 0;
        ...
        var msg = '';
        var perc = ((value - 380) / base * 10).toFixed(1);
        if (perc < 0)
            perc = 0;
        ...
        var msg = '';
        var perc = (200 + (value - 340) / 22).toFixed(1);
        if (perc < 0)
            perc = 0;
        ...
        var msg = '';
        var perc = ((value - 380) / base * 13).toFixed(1);
        if (perc < 0)
            perc = 0;
